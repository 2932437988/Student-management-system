 ...
 */
public class TripbackSaveServlet extends HttpServlet {
		...
		for(FileItem x:items) {
			try {
				if(x.isFormField()) {
					...
					if(filename.equals("backtime")) {
						backtime=x.getString("UTF-8");
			...
			try {
				...
				tripin=new Tripback(userid,DateUtils.formatString(subtime, "yyyy-MM-dd HH:mm:ss"),DateUtils.formatString(backtime, "yyyy-MM-dd HH:mm:ss"),residentialaddress,tutorapprovalvoucher,travelcard,healthcodegreencode,nucleicacidproof);
			}catch(Exception e1) {
				e1.printStackTrace();
			}
			...
			try {
				...
				saveNum=tripbackDao.addTripback(coon,tripin);
				if(saveNum>0) {
